...
class Model:
    def __init__(self,params = None):
        ...
        if params is not None: 
            ...
            if 'dt_measurements' in params:
                self.dt = params['dt_measurements']
            else:
                self.dt = 3600*24*5 # default is 5 days
            if 'obs_type' in params:
                self.obs_type = params['obs_type']
            else:
                self.obs_type = ['Temperature'] 
            if 'simul_time' in params:
                self.simul_time = params['simul_time']
            else:
                raise NameError('simul_time is not defined')
                
            if 'x_obs' in params:
                self.x_obs = params['x_obs']
            else:
                raise NameError('x_obs is not defined')
                
            if 'y_obs' in params:
                self.y_obs = params['y_obs']
            else:
                raise NameError('y_obs is not defined')
                
            if 'z_obs' in params:
                self.z_obs = params['z_obs']
            else:
                raise NameError('z_obs is not defined')
    ...
    def run_model(self,s,idx=0):
        ...
        dat.parameter.update(
            ...
            'tstop': self.simul_time,                   # stop time
        ...
        self.set_measurement_blk(dat, self.x_obs, self.y_obs, self.z_obs, nx)
        ...
        measurements = self.observation_model(sim_dir,self.obs_type)
    ...
    def set_measurement_blk(self,dat, x, y, z, nx):
        ...
        for i in x_obs:
            for j in y_obs:
                for k in z_obs:
                    block_list.append(nx[0]*nx[1]*(k-1)+nx[0]*(j-1)+i)
    ...
    def interp_obs(self,df, col, dt):
        elements = df['Element'].unique()
        elements.sort()
        obs = []
        for elem in elements:
            x = np.array(df[df['Element']==elem]['Time'])
            x_new = [dt* i for i in range(50) if dt*i>np.min(x) and dt*i<np.max(x)]
            for c in col:
                y = np.array(df[df['Element']==elem][c])
                f = interp1d(x, y)
                y_new = f(x_new)
                obs.extend(y_new)
    ...
    def observation_model(self,cur_dir,obs_type):
        ''' observation_model
        '''
        #if obs_type != ['Temperature'] and  obs_type != ['Pressure'] :
        #    raise ValueError('obs_type should be either Temperature of Gas Pressure')
        measured_data = self.read_FOFT(cur_dir)
        #measured_data = self.read_SAVE()
        obs = self.interp_obs(measured_data, obs_type, self.dt)
    ...
    def run(self,s,par,ncores=None):
        ...
        for i in range(s.shape[1]):
            simul_obs.append(self.run_model(s[:,i],idx=0))
        print('start running pyTOUGH simulations')
        if par:
        ...
        #for i in range(s.shape[1]):
        #    print('running forward model for %d th column of %d' % (i, s.shape[1]))
        #    simul_obs.append(self.run_model(s[:,i],idx=0))
        #a = np.array(simul_obs).T
        #print(a.shape)
